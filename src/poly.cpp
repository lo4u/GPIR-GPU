...
// 在多项式矩阵 a 和 b 之间进行矩阵乘法操作，并将结果存储在 out 中。这个函数特别适用于在快速傅里叶变换（NTT）域中进行多项式乘法
...
// 实现了在快速傅里叶变换（NTT）域中对多项式矩阵 a 和 b 进行矩阵乘法，并将结果存储在 out 中。与之前的 multiply 函数相比，这个函数没有在每次迭代中进行取模操作（即 "no reduce"）
...
// 用于在快速傅里叶变换（NTT）域中对两个多项式矩阵 a 和 b 进行加法操作，并将结果存储在 c 中
...
// 多项式矩阵 b 加到多项式矩阵 out 的特定位置 (t_row, t_col)
...
// 用于在快速傅里叶变换（NTT）域中对两个多项式矩阵 a 和 b 进行加法操作，返回得到的值
...
// 将一个多项式矩阵 a 中的每个系数与另一个多项式矩阵 singlePoly （仅含有一个多项式）中的系数进行逐元素相乘，并将结果存储在 out 中
...
// // 将一个多项式矩阵 a 中的每个系数与另一个多项式矩阵 singlePoly （仅含有一个多项式）中的系数进行逐元素相乘，返回结果
...
// 用于将两个多项式矩阵 a 和 b 垂直合并成一个新的矩阵 c
...
// 对多项式矩阵 a 进行一个自同态变换，并将结果存储在 out 中
...
// 将多项式矩阵 a 中的每个元素取(加法)逆（在模数意义下）。函数将结果存储在 out 中
...
// 将多项式矩阵 a 中的每个元素取(加法)逆（在模数意义下），返回得到的值
...
// 将多项式矩阵 a 从系数表示转换为快速傅里叶变换（NTT）表示，并将结果存储在 out 中，但不进行模数还原（即 "no reduce"）
...
// 将多项式矩阵 a 从系数表示转换为快速傅里叶变换（NTT）表示，并将结果存储在 out 中
...
// 将多项式矩阵 a 从系数表示转换为快速傅里叶变换（NTT）表示，返回得到的值
...
// 将多项式矩阵 a 从快速傅里叶变换（NTT）表示转换回系数表示，并将结果存储在 out 中
void from_ntt(MatPoly& out, const MatPoly& a) {
    assert(a.rows == out.rows);
    assert(a.cols == out.cols);
    assert(a.isNTT);
    assert(!out.isNTT);
    uint64_t *scratch = (uint64_t *)malloc(crt_count * poly_len * sizeof(uint64_t));
...
// 将多项式矩阵 a 从快速傅里叶变换（NTT）表示转换回系数表示，返回得到的值
...
// 创建并返回一个包含单个多项式的 MatPoly 结构体实例。这个多项式只有一个系数，即传入的 value 参数
...
// 用于将多项式矩阵 a 中指定区域的数据复制到 out 矩阵的指定位置
void cop(
...
// 将一个多项式矩阵 a 复制到另一个多项式矩阵 out 的特定起始位置 (t_row, t_col)
...
// 负责从多项式矩阵 a 中提取一个以 (t_row, t_col) 为起始位置的子矩阵，并将这个子矩阵复制到另一个多项式矩阵 out 中
...
// 负责从多项式矩阵 a 中提取一个以 (t_row, t_col) 为起始位置的子矩阵，返回该子矩阵
...
// 将一个整数数组 inp 中的数据复制到多项式矩阵 mat 的数据数组 data 中
...
// 将多项式矩阵 a 中的每个系数除以一个常数 divisor，并将结果存储在 out 中
...
// 用于将一个整数 a 从一个模数 inp_mod 重新缩放到另一个模数 out_mod
...
// 收一个多项式矩阵 a 和两个模数 inp_mod 和 out_mod，然后返回一个新的多项式矩阵，其中的每个系数都被重新缩放到 out_mod
