...
// 将一组密文从 NTT（快速傅里叶变换）域逆变换回“标准”形式，然后通过中国剩余定理（CRT）提升它们
...
// 通过第一个维度 j 的 Regev 编码扩展成第 j 个位置为 1 的单位向量的编码，
// 然后根据该编码在数据库 database 中选中第一个维度
...
void load_db(uint64_t*& B, size_t num_expansions, size_t further_dims, size_t IDX_TARGET) {
    size_t total_n = (1 << num_expansions) * (1 << further_dims);
    size_t dim0 = 1 << num_expansions; // 第一维度的总数
    size_t num_per = total_n / dim0; // 数据库总数据量 / 第一维度
...
void scalToMat(
    size_t m_conv,                  // log_z{q} + 1
    MatPoly &out_reg,               // n1 x n0 -> 3 x 2
    const MatPoly &cv,              // n0 x 1 -> 2 x 1, 编码 c
    const MatPoly &W,                // n1 x (n0 * m_conv), 转换密钥 W
    MatPoly &cv_0,                  // 编码 c 中的 c_0  
    MatPoly &cv_1,                  // 编码 c 中的 c_1
    MatPoly &cv_ntti,               // c_0 的非 NTT (标准) 形式
    MatPoly &square_cv,
    MatPoly &ginv_c,
    MatPoly &ginv_c_nttd,           // G_{n,z}^{-1}(c_0I_n)
    MatPoly &prod_W_ginv,
    MatPoly &padded_cv_1,
    MatPoly &ginv_c_raw,
...
void runConversionImproved(
    ...
    cout << "qe_first:" << qe_first << endl;
    cout << "qe_rest:" << qe_rest << endl;
    if(directly_upload_later_cts) cout << "directly uploading Regev -> GSW ciphertexts" << endl;
    cout << "directly_upload_later_cts:" << directly_upload_later_cts << endl;
    vector<MatPoly> cv_v;
    for (size_t round = 0; round < num_expansion_rounds; round++) {
        ...
        size_t g = (size_t) ceil(log2((double)( num_bits_to_gen ))); // 处理第一维度时，值为 v1 ； 否则为 
        cout << "g = " << g << endl;
        ...
        uint64_t scal_const = 1;//inv_mod(1 << g, Q_i_u128); // 2^{-r1}

        for (size_t subround = 0; subround < num_subrounds; subround++) {
            start_timing();
            MatPoly sigma(1, 1, false);
            uint64_t init_val = scale_k;//ab_i;//factor_init; // \lfloor q / p \rfloor

            if (stopround != 0) {
                // encode first dimension bits in even coeffs
                // encode rest of the scalars in odd coeffs
                sigma.data[2*idx_dim0] = (scal_const * (__uint128_t)init_val) % Q_i; // 2^{-r1} * q/p * x_{i}
            ...
            MatPoly cv = encryptSimpleRegev(sigma); // q = cv
    ...
    cout << "directly _upload_first_cts:" << directly_upload_first_cts << endl;
...
MatPoly Query(size_t num_expansions, size_t further_dims, size_t IDX_TARGET) {
    MatPoly cv;
    ...
    for (size_t round = 0; round < num_expansion_rounds; round++) {
        ...
        for (size_t subround = 0; subround < num_subrounds; subround++) {
            ...
            cv = encryptSimpleRegev(sigma); // q = cv
            time_query_gen += end_timing();
        }
    }
    return cv;
...
void ServerExpand(
    ...
    // expand the cv to (2^v1 + v2 * t_{GSW}) Regev ciphertexts
    // the result is stored in the round_cv_v
    double quick_expansion_time = expandImproved(round_cv_v, g, m_exp, W_exp_v, W_exp_right_v, ell * further_dims, stopround);
    total_time += quick_expansion_time;

    // reorder ciphertexts if using stopround
    start_timing();
    // move the even coeffs (i.e. the first dim index i (Regev ciphertexts of basic vector)) 
    // to the front of the round_cv_v
    // the rest odd coeffs (i.e. the next dims index j_{l}(v2 * t_{GSW} Regev ciphertexts))
    round_cv_v = reorderFromStopround(round_cv_v, num_expanded, ell * further_dims);
    total_time += end_timing();
    ...
    start_timing();
    // use scalToMat alogrithm to convert scalar
    // the first 2^v1 element of cv_v to matrix Regev ciphertexts
    // restore the result to C_reg, and then move them to expansionLocals.cts
    for (size_t i = 0; i < num_expanded; i++) {
        scalToMat(
            m_conv,
            C_reg,
    ...
    start_timing();
    // use regevToGSW alogrithm to convert scalar Regev ciphertexts
    // the first 2^v1 element of cv_v to GSW ciphertexts
    // restore the result to Cp, and then move them to g_Q_nttd
    // restore the no-NTT to g_Q_crtd
    for (size_t i = 0; i < further_dims; i++) {
        size_t cv_v_offset = num_expanded + i * ell;
...
MatPoly CompressWithModSwitch(MatPoly ct) {
    MatPoly ct_inp(n1, n2, false);
    MatPoly total_resp(n1, n2, false);
...
void process_crtd_query(
    ...
    FurtherDimsLocals temp = furtherDimsLocals;
    ...
    if (temp.cts[20] == furtherDimsLocals.cts[20]) {
        cout << "==========1111111111111111111==============" << endl;
    }

}

void do_test(uint64_t *B, size_t num_expansions, size_t further_dims, size_t IDX_TARGET) {
    // =====================================================================
    // do setup
    // =====================================================================
    ...
    for (size_t trial = 0; trial < num_trials; trial++) {
        ...
        print_summary(num_expansions, further_dims);
    }
}